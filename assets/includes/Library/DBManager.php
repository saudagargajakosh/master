...
class DBManager {
	...
	private $connection;
	private $connection_jp;
	private $magic_quotes_active;
	private $real_escape_string_exists;
	private function open_connection() {
		$db1 = mysqli_connect(DBH,DBU,DBPW);
		$db2 = mysqli_connect(DBH,DBU1,DBPW1);
		@$this->connection = $db1;
		@$this->connection_jp = $db2;
		// @$this->connection_jp = mysqli_connect(DBH,DBU1,DBPW1);
			if(!$this->connection && !$this->connection_jp) {
			fatal_error("connection Failed!" , mysqli_connect_error());
			}  else {
				$db_select= mysqli_select_db($this->connection,DBN);
				$db_select_jp = mysqli_select_db($this->connection_jp,DBN1);
					if(!$db_select && !$db_select_jp) {
	...
	public function query_jp($sql) {
	$result1 = mysqli_query($this->connection_jp, $sql);
	if (mysqli_num_rows($result1) > 0) {
		while($row = mysqli_fetch_array($result1)) {
			$result[] =  $row;
		}
	}
	// $result_sets = mysqli_free_result($result);
	// echo "<pre>"; print_r($returns); exit;
		$this->confirm_query_jp($result);
		return $result;
	}
	...
	private function confirm_query_jp($result) {
		if(!$result) {
			fatal_error ('Query Failed!', mysqli_error($this->connection_jp));
		}
	}
}
