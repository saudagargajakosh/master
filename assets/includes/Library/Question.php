...
class Question Extends OneClass {
	...
	public static $db_fields = array( "id" , "user_id" , "title" , "feed", "content", "likes", "dislikes", "answers" , "views" , "follows" , "slug", "created_at" , "updated_at" , "published" , "anonymous", "cat_id", "partnr_id", "funding", "pincode", "skill_ids");
	...
	public static function get_feed_for($user_id , $query , $string, $data) {
		global $db;
		...
		*/
		...
		{
			// print_r($data); exit;
			return self::preform_sql("SELECT * FROM " . DBTP . self::$table_name . " WHERE published = 1 {$str} AND {$data[1]} = {$data[0]->id} ORDER BY created_at DESC " );
		}
		else
		{
	...
	public function get_industry_id($uristring)
	{ 
		global $db;
		return self::preform_sql("SELECT * FROM `p_questions` WHERE slug = '".$uristring."'" );
	}
}
