...
/*
*  Login with mobile or email or otp (Saudagar)
*/
...
$(function(){
	...
	$("#continue").on('click', function(e){
	    ...
	    else
	    {
	    	if(isNaN(em_mob))
	    	{
	    		valid = validateEmail(em_mob);
	    	}
	    	else
	    	{
	    		valid = em_mob;
	    		$(".loginWithOtp").removeClass('otp_login');
	    	}
	    	...
	    	{
	    		$('.email_error').text('* You entered wrong email address or mobile number.').css({"display":"block"});
	    	}
	    }
	});
	...
	if(livelihood != '' || livelihood == 'livelihood')
	{
		$("#livelihoodlogin").modal('show');
	}

	if(form_errors1 != '' || form_errors1 == 'form_errors')
	{
		$(".danger-text").text('Please fill all mandatory details for register.');
		$("#login .nav-tabs li").first().removeClass('active');
		$("#uploadTab").removeClass('active');
		$("#browseTab").addClass('active');
		$("#login .nav-tabs li").next().addClass('active');
		$("#login").modal('show');
	}

	if(m_error != '' || m_error == 'merror')
	{
		$("#login").modal('show');
	}
	else if(m_error != '' || m_error == 'msuccess')
	{
		$("#login").modal('show');
	}
	else if(m_otp != '' || m_otp == 'otp-error')
	{
		$("#login").modal('show');
		$(".firstContent").hide();
		$(".nav-tabs").hide();
	...
	$("#email_phone").keyup(function(){
		if($.isNumeric($("#email_phone").val()))
		{
			$(".otp_login").removeClass('otp_login');
		}
	});

	$(".otpLogin").on('click', function(e){
		e.preventDefault();
		$(".firstContent").hide();
		$(".nav-tabs").hide();
		$(".secondContent").show();
		var mobile = $('#email_phone').val();

		if(!isNaN(mobile))
		{
			var updateOTP = $.ajax({
			      type: 'POST',
			      url: baseUrl+"user/generateOTP",
			      data: {'mobile':mobile},
			      dataType: "json",
	...
	$(document).on('click', '.resend_OTP', function(e){
		...
		if(!isNaN(mobile))
		{
			var updateOTP = $.ajax({
			      type: 'POST',
			      url: baseUrl+"user/get_resend_OTP",
			      data: {'mobile':mobile},
			      dataType: "json",
			      success: function(resultData){ 
			      	alert(resultData);
	...
	// Employer User 
	$("#continueEmp").on('click', function(e){
	    e.preventDefault();
	    var valid = '';
	    var em_mob = $('#email_phoneEmp').val();
	...
	if(m_errorEmp != '' || m_errorEmp == 'emperror')
	{
		$("#loginEmployer").modal('show');
	}
	else if(m_errorEmp != '' || m_errorEmp == 'empsuccess')
	{
		$("#loginEmployer").modal('show');
	}

	if(form_errorsEmp != '' || form_errorsEmp == 'form_errorsEmp')
	{
		$(".empErrors").text('Please fill all mandatory details for register.');
		$("#loginEmployer .nav-tabs li").first().removeClass('active');
		$("#loginEmpTab").removeClass('active');
		$("#registerEmpTab").addClass('active');
		$("#loginEmployer .nav-tabs li").next().addClass('active');
		$("#loginEmployer").modal('show');
	}

	$(".otpLoginEmp").on('click', function(e){
		e.preventDefault();
		$(".firstContentEmp").hide();
		$(".nav-tabs").hide();
		$(".secondContentEmp").show();
		var mobile = $('#email_phoneEmp').val();
		if(!isNaN(mobile))
		{
			var updateOTP = $.ajax({
			      type: 'POST',
			      url: baseUrl+"user_employer/generateOTPEmp",
			      data: {'mobile':mobile},
			      dataType: "json",
			      success: function(resultData){ 
			      	alert(resultData);
	...
	$(document).on('click', '.resend_OTPEmp', function(e){
		e.preventDefault();
		var mobile = $('#email_phoneEmp').val();
		if(!isNaN(mobile))
		{
			var updateOTP = $.ajax({
			      type: 'POST',
			      url: baseUrl+"user_employer/get_resend_OTPEmp",
			      data: {'mobile':mobile},
			      dataType: "json",
			      success: function(resultData){ 
			      	alert(resultData);
...
/*
* ends Login with mobile or email or otp
*/