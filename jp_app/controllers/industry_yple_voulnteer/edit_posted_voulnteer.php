...
class Edit_Posted_Voulnteer extends CI_Controller {
	...
	{
		if($id==''){
			redirect(base_url().'industry_yple_voulnteer/dashboard','');
			exit;
		}	
		...
		$data['title'] = SITE_NAME.' : Edit Posted voulnteer';
		$data['msg']='';
		...
		$row = $this->posted_voulnteer_model->get_posted_job_by_id_voulnteer_id($id, $this->session->userdata('user_id'));
		...
		if ($this->form_validation->run() === FALSE) {
			$this->load->view('industry_yple_voulnteer/edit_posted_voulnteer_view',$data);
			return;
		}
		...
		$this->posted_voulnteer_model->update_posted_job($id, $job_array);
		...
		redirect(base_url('industry_yple_voulnteer/edit_posted_industry_yple_voulnteer/'.$id),'');		
	}
