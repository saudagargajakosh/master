...
class Post_New_Voulnteer extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = SITE_NAME.' : Post New Voulnteer';
		$data['msg']='';
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->load->view('industry_yple_voulnteer/post_new_voulnteer_view',$data);
			return;
		...
		$job_id = $this->posted_voulnteer_model->add_posted_job($job_array);
		$job_slug = $this->make_job_slug($row->company_slug, $this->input->post('job_title'), $this->input->post('city'), $job_id);
		$this->posted_voulnteer_model->update_posted_job($job_id, array('job_slug' => $job_slug));
		...
		redirect(base_url('industry_yple_voulnteer/my_posted_voulnteer'),'');
