...
class Company_Training extends CI_Controller {
	...
	public function index()
	{
		...
		//Jobs by company
		$result_posted_jobs = $this->posted_training_model->get_active_posted_training_by_company_id($row_company->ID, 100, 0);
		...
		$job_url = $row_company->company_slug.'-training-in-';
		...
		$this->load->view('company_training_view',$data);
	}
	
	public function is_already_applied_for_job($user_id='', $job_id){
		$is_already_applied = '';
		if($this->session->userdata('is_job_seeker')==TRUE){
			$is_already_applied = $this->applied_training_model->get_applied_job_by_seeker_and_job_id($user_id, $job_id);
