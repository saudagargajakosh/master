...
class Edit_Posted_Training extends CI_Controller {
	...
	{
		if($id==''){
			redirect(base_url().'industry_yple_training/dashboard','');
			exit;
		}	
		...
		$data['title'] = SITE_NAME.' : Edit Posted training';
		$data['msg']='';
		...
		$row = $this->posted_training_model->get_posted_job_by_id_training_id($id, $this->session->userdata('user_id'));
		...
		if ($this->form_validation->run() === FALSE) {
			$this->load->view('industry_yple_training/edit_posted_training_view',$data);
			return;
		}
		...
		$this->posted_training_model->update_posted_job($id, $job_array);
		$this->add_skill($required_skills);
		$this->session->set_flashdata('msg', '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>Success!</strong> training has been updated successfully.</div>');
		redirect(base_url('industry_yple_training/edit_posted_industry_yple_training/'.$id),'');		
	}
