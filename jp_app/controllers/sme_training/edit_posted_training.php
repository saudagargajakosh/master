...
class Edit_Posted_Training extends CI_Controller {
	...
	{
		if($id==''){
			redirect(base_url().'training/dashboard','');
			exit;
		}	
		...
		if(!$row){
			redirect(base_url().'sme_training/dashboard','');
			exit;	
		}
		$data['row'] = $row;
		...
		if ($this->form_validation->run() === FALSE) {
			$this->load->view('sme_training/edit_posted_training_view',$data);
			return;
		}
		...
		redirect(base_url('sme_training/edit_posted_training/'.$id),'');		
	}
