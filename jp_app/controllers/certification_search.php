...
class Certification_Search extends CI_Controller {
	...
	public function index()
	{
		...
		//Pagination starts
		$total_rows = $this->posted_certification_model->count_searched_certification_records($param,$param2);
		$config = pagination_configuration(base_url("search-certification/".$this->uri->segment(2)), $total_rows, 20, 3, 5, true);
		...
		$obj_result = $this->posted_certification_model->get_searched_certification($param,$param2, $config["per_page"], $page);
		// print_r($obj_result);
		//Left Side Starts
		...
		$this->load->view('certification_search_view',$data);
	}
	...
	{
		//Group By Title
		$title_group = $this->posted_certification_model->get_searched_group_by_title($param);
		
		//Group By City
		$city_group = $this->posted_certification_model->get_searched_group_by_city($param);
		
		//Group By Companies
		$company_group = $this->posted_certification_model->get_searched_group_by_company($param);
		//Group By Salary Range
		// $salary_range_group = $this->posted_certification_model->get_searched_group_by_salary_range($param);
	...
	public function is_already_applied_for_job($user_id='', $job_id){
		...
		if($this->session->userdata('is_job_seeker')==TRUE){
			$is_already_applied = $this->applied_certification_model->get_applied_job_by_seeker_and_job_id($user_id, $job_id);
