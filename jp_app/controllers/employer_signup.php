...
class Employer_Signup extends CI_Controller {
	...
	public function index()
	{
		...
		$this->form_validation->set_rules('emailEmp', 'Email', 'trim|required|valid_email|is_unique[pp_employers.email]|strip_all_tags');	
		$this->form_validation->set_rules('passEmp', 'Password', 'trim|required|min_length[6]|strip_all_tags');
		$this->form_validation->set_rules('confirm_passEmp', 'Confirm password', 'trim|required|matches[pass]|strip_all_tags');
		$this->form_validation->set_rules('full_nameEmp', 'Your name', 'trim|required|strip_all_tags');
		...
		$this->form_validation->set_rules('mobile_phoneEmp', 'Mobile', 'trim|required|strip_all_tags|max_length[10]|is_unique[pp_employers.mobile_phone]');
		...
		$f_name = $this->input->post('full_nameEmp');
		$emailEmp = $this->input->post('emailEmp');
		$passEmp = md5($this->input->post('passEmp'));
		$mobile_phoneEmp = $this->input->post('mobile_phoneEmp');
		$industry_id = $this->input->post('industry_id');
		if(empty($f_name) || empty($emailEmp) || empty($passEmp) || empty($mobile_phoneEmp) || empty($industry_id))
		{
			redirect(base_url().'?errorEmp=form_errorsEmp');
		}
		...
		$key=$this->input->post('pass');
		$employer_array = array(
								'first_name' => $f_name,
								'email' => $emailEmp,
								'pass_code' => $passEmp,
							
								'mobile_phone' => $mobile_phoneEmp,
		...
		$company_array = array(
								...
								'industry_com_id' => $this->input->post('industry_id'),
		...
		$smsmessage = "Welcome, ".$this->input->post('full_nameEmp')."\r\n";
		$smsmessage .= "Thank you for joining us. Please note your profile details for future record.\r\n";
		$smsmessage .= "Username: ".$emailEmp." OR ".$mobile_phoneEmp."\r\n";
		$smsmessage .= "Password: ".$this->input->post('passEmp')."\r\n";
		$smsmessage .= "Regards";

		$smsfrom = '';
		$mobile_to_sms = $mobile_phoneEmp;
		$response_sms   = send_sms($smsfrom,$mobile_to_sms,$smsmessage);
		...
		$mail_message = $this->email_drafts_model->employer_signup($row_email->content, $employer_array, $this->input->post('pass'));
