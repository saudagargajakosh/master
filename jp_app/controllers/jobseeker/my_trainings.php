...
class My_Trainings extends CI_Controller {
	...
	public function index()
	{
		...
		//Pagination starts
		$total_rows = $this->applied_training_model->count_applied_training_jobseeker_id($this->session->userdata('user_id'));
		$config = pagination_configuration(base_url("jobseeker/my_trainings"), $total_rows, 50, 3, 5, true);
		...
		$result_applied_jobs = $this->applied_training_model->get_applied_training_by_jobseeker_id($this->session->userdata('user_id'), $config["per_page"], $page);
		$data['result_applied_jobs']= $result_applied_jobs;
		$this->load->view('jobseeker/my_trainings_view',$data);
	}
