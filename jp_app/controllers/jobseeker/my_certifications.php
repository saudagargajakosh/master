...
class My_Certifications extends CI_Controller {
	...
	public function index()
	{
		...
		//Pagination starts
		$total_rows = $this->applied_certification_model->count_applied_certifications_jobseeker_id($this->session->userdata('user_id'));
		...
		$result_applied_jobs = $this->applied_certification_model->get_applied_certifications_by_jobseeker_id($this->session->userdata('user_id'), $config["per_page"], $page);
		$data['result_applied_jobs']= $result_applied_jobs;
		$this->load->view('jobseeker/my_certifications_view',$data);
	}
