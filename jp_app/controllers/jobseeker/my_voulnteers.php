...
class My_Voulnteers extends CI_Controller {
	...
	public function index()
	{
		...
		//Pagination starts
		$total_rows = $this->applied_voulnteer_model->count_applied_voulnteer_jobseeker_id($this->session->userdata('user_id'));
		...
		$result_applied_jobs = $this->applied_voulnteer_model->get_applied_voulnteer_by_jobseeker_id($this->session->userdata('user_id'), $config["per_page"], $page);
		$data['result_applied_jobs']= $result_applied_jobs;
		$this->load->view('jobseeker/my_voulnteers_view',$data);
	}
