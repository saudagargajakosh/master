...
class Apply_Training extends CI_Controller {
	...
	public function index()
	{
		...
		$row = $this->posted_training_model->get_active_posted_training_by_id($this->input->post('jid'));
		...
		$is_already_applied = $this->applied_training_model->get_applied_job_by_seeker_and_job_id($this->session->userdata('user_id'), $this->input->post('jid'));
		
		if($is_already_applied>0){
			echo 'You have already applied for this training  has been closed.';
			exit;	
		}
		...
		$this->applied_training_model->add_applied_job($job_array);
		...
		$data_array = $this->posted_training_model->get_active_posted_training_by_id($this->input->post('jid'));
