...
class Post_New_Certification extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = SITE_NAME.' : Post New certification';
		$data['msg']='';
		...
		$data['result_salaries'] = $this->certification_price_model->get_all_records();
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->load->view('industry_yple_certification/post_new_certifcation_view',$data);
			return;
		...
		$job_id = $this->posted_certification_model->add_posted_job($job_array);
		$job_slug = $this->make_job_slug($row->company_slug, $this->input->post('job_title'), $this->input->post('city'), $job_id);
		$this->posted_certification_model->update_posted_job($job_id, array('job_slug' => $job_slug));
		
		//$this->add_skill($required_skills);
		$this->session->set_flashdata('msg', '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>Success!</strong> New certification has been posted successfully.</div>');
		redirect(base_url('industry_yple_certification/my_posted_certification'),'');
