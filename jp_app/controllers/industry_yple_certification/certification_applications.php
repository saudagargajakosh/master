...
class Certification_Applications extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = SITE_NAME.': List of Received Certification Applications';
		...
		$config = pagination_configuration(base_url("industry_yple_certification/job_applications"), $total_rows, 50, 3, 5, true);
		...
		$result_applied_jobs = $this->applied_certification_model->get_applied_certification_by_employer_id($this->session->userdata('user_id'), $config["per_page"], $page);
		$data['result_applied_jobs']= $result_applied_jobs;
		$this->load->view('industry_yple_certification/certification_applications_view',$data);
	}
