...
class Voulnteer_Search extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = $param.$searched_title.' voulnteer';
	...
	{
		//Group By Title
		$title_group = $this->posted_voulnteer_model->get_searched_group_by_title($param);
		
		//Group By City
		$city_group = $this->posted_voulnteer_model->get_searched_group_by_city($param);
		
		//Group By Companies
		$company_group = $this->posted_voulnteer_model->get_searched_group_by_company($param);
		//Group By Salary Range
		$salary_range_group = $this->posted_voulnteer_model->get_searched_group_by_salary_range($param);
	...
	public function is_already_applied_for_job($user_id='', $job_id){
		...
		if($this->session->userdata('is_job_seeker')==TRUE){
			$is_already_applied = $this->applied_voulnteer_model->get_applied_job_by_seeker_and_job_id($user_id, $job_id);
