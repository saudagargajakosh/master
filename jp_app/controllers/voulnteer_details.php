...
class Voulnteer_Details extends CI_Controller {
	...
	public function index()
	{
		...
		$row_posted_job = $this->posted_voulnteer_model->get_active_posted_voulnteer_by_id($job_id);
		if(!$row_posted_job){
			$this->load->view('404_view',$data);	
			return;
		}
		$currently_opened_jobs = $this->posted_voulnteer_model->count_active_opened_voulnteer_by_company_id($row_posted_job->CID);
		...
		if($this->session->userdata('is_user_login')==TRUE){
			$is_already_applied = $this->applied_voulnteer_model->get_applied_job_by_seeker_and_job_id($this->session->userdata('user_id'), $job_id);
		...
		$this->load->view('job_voulnteer_view',$data);
	}
