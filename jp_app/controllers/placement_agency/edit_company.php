...
class Edit_Company extends CI_Controller {
	...
	public function index()
	{
		...
		$this->form_validation->set_rules('email', 'Your email', 'trim|required|strip_all_tags');
		...
		$this->form_validation->set_rules('aadhar_no', 'Aadhar No', 'trim|required|strip_all_tags');
	
		$this->form_validation->set_rules('register_no', 'Register No', 'trim|required|strip_all_tags');
		$this->form_validation->set_rules('pincode', 'pincode', 'trim|required|strip_all_tags');
		...
		$data['aadhar_no'] = (set_value('aadhar_no'))?set_value('aadhar_no'):$row->aadhar_no;
		$data['register_no'] = (set_value('register_no'))?set_value('register_no'):$row->register_no;
		$data['pincode'] = (set_value('pincode'))?set_value('pincode'):$row->pincode;
		$data['email'] = (set_value('email'))?set_value('email'):$row->email;
		$data['taluka'] = (set_value('taluka'))?set_value('taluka'):$row->taluka;
		$data['state'] = (set_value('state'))?set_value('state'):$row->state;
		...
		if ($this->form_validation->run() === FALSE) {
			$this->load->view('placement_agency/edit_company_profile_view',$data);
			return;
		}
		...
		redirect(base_url('placement_agency/edit_company'));
	}
		public function pincode_details()
    {
       $pincode = $this->input->post('pincode');
       $pincodedetails = $this->employers_model->pincode_details($pincode);
	
       echo json_encode(array('pincode'=>$pincodedetails));
    }
