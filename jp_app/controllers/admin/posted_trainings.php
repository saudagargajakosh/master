...
class Posted_Trainings extends CI_Controller {
	public function index(){
		$data['title'] = SITE_NAME.': Manage Posted Jobs';
		$data['msg'] = '';
		
		//Pagination starts
		$total_rows = $this->posted_jobs_model->record_count('pp_post_training');
		$config = pagination_configuration(base_url("admin/posted_trainings"), $total_rows, 50, 3, 5, true);
		...
		$this->load->view('admin/posted_training_view', $data);
		return;
	}
	...
	public function details($id=''){
		...
		if($id==''){
			redirect(base_url().'admin/posted_trainings','');
			exit;
		}
		$data['title'] = SITE_NAME.': Posted trainings Details';
		$data['msg'] = '';
		$obj_row = $this->posted_training_model->get_posted_training_by_id($id);
		$data['row'] = $obj_row;
		$this->load->view('admin/posted_training_details_view', $data);
		return;
	}
	...
	public function status($id='', $current_staus=''){
		...
		$this->posted_training_model->update_posted_job($id, $data);
		echo $new_status;
		exit;
	}	
	...
	public function delete_posted_training($id=''){
		...
		$obj_row = $this->posted_training_model->get_posted_training_by_id($id);
		$this->posted_training_model->delete_posted_job($obj_row->ID);
	...
	public function training_by_company($employer_id=''){
		...
		//Pagination starts
		$total_rows = $this->posted_jobs_model->count_records('pp_post_training','employer_ID',$employer_id);
		$config = pagination_configuration(base_url("admin/posted_trainings/training_by_company/"), $total_rows, 50, 3, 5, true);
		...
		$obj_result = $this->posted_training_model->get_posted_training_by_employer_ID($employer_id, $config["per_page"], $page);
		...
		$data['result'] = $obj_result;
		$this->load->view('admin/posted_training_by_company_view', $data);
		return;
	}
	...
	public function job_activation_email_to_employer($job_id){
		...
		$row = $this->posted_training_model->get_posted_job_by_job_id($job_id);
