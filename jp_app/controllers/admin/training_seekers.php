...
class Training_Seekers extends CI_Controller {
	public function index(){
		...
		$config = pagination_configuration(base_url("admin/training_seekers"), $total_rows, 50, 3, 5, true);
		...
		$this->load->view('admin/training_seekers_view', $data);
		return;
	}
	...
	public function details($id=''){
		...
		if($id==''){
			redirect(base_url().'admin/training_seekers','');
			exit;
		}
		$data['title'] = SITE_NAME.': Training Details';
		$data['msg'] = '';
		$obj_row = $this->job_seekers_model->get_job_seeker_by_id($id);
		$obj_result = $this->job_seekers_model->get_all_applied_training_by_seekers_ID($id,50,0);
		//print_r($obj_result);
		$data['result'] = $obj_result;
		$data['row'] = $obj_row;
		$this->load->view('admin/training_seeker_details_view', $data);
		return;
	}
	...
	public function search($search_name='nm_', $search_email='em_', $search_gender='ge_', $search_city='ct_'){
		...
		$config = pagination_configuration_search(base_url("admin/training_seekers/search/".$making_url.'/?'), $total_rows, 50, $segment, 5, true);
		...
		if ($_POST)
			redirect(base_url('admin/training_seekers/search/'.$making_url));
		else
			$this->load->view('admin/trainng_seekers_view', $data);
		return;
	}
	
	public function applied_training_list($seeker_id=''){
		$data['title'] = SITE_NAME.': Manage Jobseeker Applications';
		$data['msg'] = '';
		
		//Pagination starts
		$total_rows = $this->job_seekers_model->count_records('pp_seeker_applied_for_training', 'seeker_ID', $seeker_id);
		$config = pagination_configuration(base_url("admin/training_seekers/applied_training_list"), $total_rows, 50, 3, 5, true);
		...
		$obj_result = $this->job_seekers_model->get_all_applied_training_by_seekers_ID($seeker_id, $config["per_page"], $page);
		$data['result'] = $obj_result;
		$this->load->view('admin/applied_training_list_view', $data);
		return;
	}
