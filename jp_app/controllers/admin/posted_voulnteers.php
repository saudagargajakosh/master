...
class Posted_voulnteers extends CI_Controller {
	public function index(){
		$data['title'] = SITE_NAME.': Manage Posted Jobs';
		$data['msg'] = '';
		
		//Pagination starts
		$total_rows = $this->posted_voulnteer_model->record_count('pp_post_voulnteer');
		$config = pagination_configuration(base_url("admin/posted_voulnteers"), $total_rows, 50, 3, 5, true);
		...
		$row = $this->posted_voulnteer_model->get_posted_voulnteer_by_job_id();
		...
		$this->load->view('admin/posted_voulnteer_view', $data);
		return;
	}
	...
	public function details($id=''){
		...
		$data['msg'] = '';
		$obj_row = $this->posted_voulnteer_model->get_posted_voulnteer_by_id($id);
		$data['row'] = $obj_row;
		$this->load->view('admin/posted_voulnteer_details_view', $data);
		return;
	}
	...
	public function status($id='', $current_staus=''){
		...
		$this->posted_voulnteer_model->update_posted_job($id, $data);
		echo $new_status;
		exit;
	}	
	...
	public function delete_posted_voulnteer($id=''){
		...
		$obj_row = $this->posted_voulnteer_model->get_posted_voulnteer_by_id($id);
		$this->posted_voulnteer_model->delete_posted_job($obj_row->ID);
	...
	public function search($search_title='te_', $search_company='co_', $search_city='ct_', $search_featured='fe_', $search_sts='st_'){
		...
		if($search_title=='te_' && $search_company=='co_' && $search_city=='ct_' && $search_featured=='fe_' && $search_sts=='st_'){
			redirect(base_url('admin/posted_voulnteers'));
			return;
		}
		$new_array = array();
		...
		$config = pagination_configuration_search(base_url("admin/posted_voulnteers/search/".$making_url.'/?'), $total_rows, 50, $segment, 5, true);
		...
		if ($_POST)
			redirect(base_url('admin/posted_voulnteers/search/'.$making_url));
		else
			$this->load->view('admin/posted_voulnteer_view', $data);
		return;
	}
	
	public function voulnteer_by_company($employer_id=''){
		...
		//Pagination starts
		$total_rows = $this->posted_voulnteer_model->count_records('pp_post_voulnteer','employer_ID',$employer_id);
	
		$config = pagination_configuration(base_url("admin/posted_voulnteers/voulnteer_by_company/"), $total_rows, 50, 3, 5, true);
		...
		$obj_result = $this->posted_voulnteer_model->get_posted_voulnteer_by_employer_ID($employer_id, $config["per_page"], $page);
		...
		$data['result'] = $obj_result;
		$this->load->view('admin/posted_voulnteer_by_company_view', $data);
		return;
	}
	...
	public function job_activation_email_to_employer($job_id){
		...
		$row = $this->posted_voulnteer_model->get_posted_voulnteer_by_job_id($job_id);
