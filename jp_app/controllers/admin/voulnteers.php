...
class Voulnteers extends CI_Controller {
	public function index(){
		...
		$config = pagination_configuration(base_url("admin/voulnteers"), $total_rows, 50, 3, 5, true);
// print_r($config);
		...
		$this->load->view('admin/voulnteer_view', $data);
		return;
	}
	...
	public function search(){
		...
		if($search_name=='' && $search_email=='' && $search_company=='' && $search_city=='' && $top_employer=='' && $industry_ID==''){
			redirect(base_url('admin/voulnteers'));
			return;
		}
		$new_array = array();
		$search_data = array();
		...
		$config = pagination_configuration_search(base_url("admin/voulnteers/search/?".$url_params), $total_rows, 50, 3, 5, true);
