...
class Posted_Certifications extends CI_Controller {
	public function index(){
		$data['title'] = SITE_NAME.': Manage Posted Jobs';
		$data['msg'] = '';
		
		//Pagination starts
		$total_rows = $this->posted_certification_model->record_count('pp_post_certification');
		$config = pagination_configuration(base_url("admin/posted_certification"), $total_rows, 50, 3, 5, true);
		...
		$this->load->view('admin/posted_certification_view', $data);
		return;
	}
	...
	public function details($id=''){
		...
		$data['msg'] = '';
		$obj_row = $this->posted_certification_model->get_posted_certification_by_id($id);
		$data['row'] = $obj_row;
		$this->load->view('admin/posted_certification_details_view', $data);
		return;
	}
	...
	public function status($id='', $current_staus=''){
		...
		$this->posted_certification_model->update_posted_job($id, $data);
		echo $new_status;
		exit;
	}	
	...
	public function delete_posted_certification($id=''){
		...
		$obj_row = $this->posted_certification_model->get_posted_certification_by_id($id);
		$this->posted_certification_model->delete_posted_job($obj_row->ID);
	...
	public function certification_by_company($employer_id=''){
		
		$data['title'] = SITE_NAME.': Manage Posted Jobs';
		$data['msg'] = '';
		
		if($employer_id==''){
			redirect(base_url().'admin/posted_certification','');
			exit;
		}
		
		//Pagination starts
		$total_rows = $this->posted_certification_model->count_records('pp_post_certification','employer_ID',$employer_id);
	
		$config = pagination_configuration(base_url("admin/posted_certifications/jobs_by_company/"), $total_rows, 50, 3, 5, true);
		...
		$obj_result = $this->posted_certification_model->get_posted_certification_by_employer_ID($employer_id, $config["per_page"], $page);
		...
		$data['result'] = $obj_result;
		$this->load->view('admin/posted_certification_by_company_view', $data);
		return;
	}
	...
	public function job_activation_email_to_employer($job_id){
		...
		$row = $this->posted_certification_model->get_posted_job_by_job_id($job_id);
