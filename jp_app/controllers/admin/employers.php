...
class Employers extends CI_Controller {
	public function index(){
		...
		// echo "<pre>"; print_r($obj_result); exit;
...
// print_r(get_declared_classes());
	$data['title'] = SITE_NAME.': Manage Jobseekers';
		$data['msg'] = '';
		if($this->input->post('uploadstatus')){
		
			
					include(FCPATH.'/jp_app/third_party/PHPExcel/IOFactory.php');
	...
	$inser_id=$this->companies_model->add_company_excel($insertdata_com);							
                 	$insertdata= array(
									// 'batch_id'=>$i,
									// 'ID'=>$r['A'],
									'email'=>$r['A'],
									'pass_code'=> $this->encrypt->encode($r['B']),
									'first_name'=> $r['C'],
									'last_name'=> $r['D'], 
									'country'=> $r['E'],
									'city'=> $r['F'],
									'mobile_phone'=> $this->encrypt->encode($r['G']), 
									'gender'=> $r['H'], 
									'dob'=> $r['I'], 
									'home_phone'=> $r['J'], 
									'present_address'=> $r['K'], 
								...
								$insertdata['company_ID'] = $inser_id;
								$inser_id=$this->employers_model->add_employer_excel($insertdata);
					...
					if ($inser_id > 0) {
						// $this->userActivity("Student temporary data uploaded by ".$data['user']['name']);
						$this->session->set_flashdata('success', 'employer entries uploaded successfully');
					}
					else 
						$this->session->set_flashdata('error', 'Unable to upload  employer');
