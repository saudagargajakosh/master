...
// print_r(get_declared_classes());
	$data['title'] = SITE_NAME.': Manage Jobseekers';
		...
		if($this->input->post('uploadstatus')){
						...
						if ($i++ != 0) {			
						    if ($valid == 1) {
								$insertdata= array(
									...
									'last_name'=> $r['B'],
									'email'=> $r['C'],
									'password'=> $this->encrypt->encode($r['D']), 
									'present_address'=> $r['E'],
									'mobile'=> $this->encrypt->encode($r['F']),
									'nationality'=> $r['G'], 
									'dob'=> $r['H'], 
									'gender'=> $r['I'], 
									'city'=> $r['J'], 
								...
								$inser_id=$this->job_seekers_model->add_job_student($insertdata);
							}
							else
								break;
						}
					...
					if ($inser_id > 0) {
						...
						$this->session->set_flashdata('success', 'student entries uploaded successfully');
					}
					else 
						$this->session->set_flashdata('error', 'Unable to upload students');
	...
	public function update($id=''){
		...
		$key2=$this->input->post('mobile');
		$job_seeker_array = array(
								'first_name' => $this->input->post('first_name'),
								'last_name' => '',
								'email' => $this->input->post('email'),
								'password' => $this->encrypt->encode($key),
								'dob' => $this->input->post('dob'),
								'mobile' => $this->encrypt->encode($key2),
