...
class Jobseeker_Signup_Main extends CI_Controller {
	...
	public function index()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			redirect(base_url().'?error=form_errors');
		...
		$password = $this->load->helper('phpass');
	    $hasher = new PasswordHash(PHPASS_HASH_STRENGTH, PHPASS_HASH_PORTABLE);
	    // $password = $hasher->get_random(10);
	    // echo $password; exit;
	    ...
	    $liveli_user = array(
			...
			'email' => $this->input->post('email'),
			'mobile' => $this->input->post('mobile_number'),
			'f_name' => $this->input->post('full_name'),
			'joined' => $current_date,
			'deleted' => 0
			
		);

		// $res = $this->PasswordHash->HashPassword($this->input->post('pass'));
		$hashPass = $this->job_seekers_model->create_livelihood_user($liveli_user);
		...
		$liveli_user['user_idyple'] = $seeker_id;
		$hashPass = $this->job_seekers_model->create_livelihood_user($liveli_user);
// Thank you for joining us. Please note your profile details for future record.

// Username: singh.sheshank@gmail.com
// Password: demo123

// Regards
		$smsmessage = "Welcome, ".$this->input->post('full_name')."\r\n";
		$smsmessage .= "Thank you for joining us. Please note your profile details for future record.\r\n";
		$smsmessage .= "Username: ".$this->input->post('email')." OR ".$this->input->post('mobile_number')."\r\n";
		$smsmessage .= "Password: ".$this->input->post('pass')."\r\n";
		$smsmessage .= "Regards";

		$smsfrom = '';
		$mobile_to_sms = $this->input->post('mobile_number');
		...
		$mail_message = $this->email_drafts_model->jobseeker_signup($row_email->content, $job_seeker_array, $this->input->post('pass'));
