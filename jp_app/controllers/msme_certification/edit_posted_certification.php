...
class Edit_Posted_Certification extends CI_Controller {
	...
	{
		if($id==''){
			redirect(base_url().'certification/dashboard','');
			exit;
		}	
		...
		$data['title'] = SITE_NAME.' : Edit Posted certification';
		$data['msg']='';
		...
		$row = $this->posted_certification_model->get_posted_job_by_id_certification_id($id, $this->session->userdata('user_id'));

		if(!$row){
			redirect(base_url().'msme_certification/dashboard','');
			exit;	
		}
		$data['row'] = $row;
		...
		if ($this->form_validation->run() === FALSE) {
			$this->load->view('msme_certification/edit_posted_certification_view',$data);
			return;
		}
		...
		$this->posted_certification_model->update_posted_job($id, $job_array);
		$this->add_skill($required_skills);
		$this->session->set_flashdata('msg', '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>Success!</strong> Certification has been updated successfully.</div>');
		redirect(base_url('msme_certification/edit_posted_certification/'.$id),'');		
	}
