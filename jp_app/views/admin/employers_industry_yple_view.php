...
<aside class="right-side"> 
  ...
  <section class="content-header">
    <h1> Employers Industry Management 
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            ...
            <!--Pagination--><?php if($this->session->flashdata('error')){ ?>
            ...
            <?php } ?>
            <form id="createform" role="form" method="POST" action="<?php echo base_url('admin/employers/student')?>" enctype="multipart/form-data" style="padding-left:30px;">
              <div class="form-body">
                <div class="row">
                  <input type="hidden" name="uploadstatus" value="1"/>
                  <div class="col-md-11 form-group">
                      <label>Upload employer data in xls <a href="<?=base_url('public/excel/sample/employer_industry_yple.xls')?>" target="_blank" download><font color="red">Export sample excel sheet</font></a></label>
          ...
          <div class="box-body table-responsive">
            <?php $this->load->view('admin/common/employer_industry_yple_quick_search_bar'); ?>
            ...
            <table id="example2" class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <th>Email Address</th>
                  <th>Posted Jobs</th>
                  <th>Posted certification</th>
                  <th>Posted voulnteer</th>
                  <th>Posted training</th>
              ...
              <tbody>
				...
				if($result):
					...
					if($row->industry_com_id==6):
						$json_row = array();
						$total_posted_jobs = $this->posted_jobs_model->count_records('pp_post_jobs','employer_id',$row->ID);
						$total_posted_certification = $this->posted_certification_model->count_records('pp_post_certification','employer_id',$row->ID);
						$total_posted_voulnteer = $this->posted_voulnteer_model->count_records('pp_post_voulnteer','employer_id',$row->ID);
						$total_posted_training = $this->posted_training_model->count_records('pp_post_training','employer_id',$row->ID);
                ...
                <tr id="row_<?php echo $row->ID;?>">
                  ...
                  <td valign="middle"><a href="<?php echo base_url('admin/employers_industry_yple/details/'.$row->ID);?>"><?php echo $row->first_name.' '.$row->last_name;?></a></td>
                  ...
                  <td valign="middle"><a class="btn btn-primary btn-xs" href="<?php echo base_url('admin/posted_certifications/certification_by_company/'.$row->ID);?>" target="_blank">View (<?php echo $total_posted_certification;?>)</a>
                  <td valign="middle"><a class="btn btn-primary btn-xs" href="<?php echo base_url('admin/posted_voulnteers/voulnteer_by_company/'.$row->ID);?>" target="_blank">View (<?php echo $total_posted_voulnteer;?>)</a></td>
				  <td valign="middle"><a class="btn btn-primary btn-xs" href="<?php echo base_url('admin/posted_trainings/training_by_company/'.$row->ID);?>" target="_blank">View (<?php echo $total_posted_training;?>)</a></td>
				  <td align="center" valign="middle"><a href="<?php echo base_url('admin/employers_industry_yple/details/'.$row->ID);?>" target="_blank">
                  ...
                  <td valign="middle"><a href="<?php echo base_url('admin/employers_industry_yple/update/'.$row->ID);?>" class="btn btn-primary btn-xs">Edit Employer</a><br /><a target="_blank" href="<?php echo base_url('admin/employers_industry_yple/login/'.$row->ID);?>" class="btn btn-primary btn-xs" style="margin:1px;">Login as Employer</a><br /><a href="javascript:delete_employer(<?php echo camelize($row->ID);?>);" class="btn btn-danger btn-xs">Delete Employer</a></td>
                </tr>
                <?php endif; ?>
                <?php endforeach; else:?>
                <tr>
