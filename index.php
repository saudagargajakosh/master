...
<div class="row">
	...
	<div class="col-md-7">
		...
		if(isset($_GET['feed']) && $_GET['feed'] != '' ) {
			...
			echo '<h3 class="page-subheader name" style="margin:0">'. $lang['index-search-questions'] . ': '. $db->escape_value($_GET['feed']).$breadcrumb;
		...
		{
			$uri = $uri_segment[6];
			if($uri == 'donors' || $uri == 'associate' || $uri == 'micro-finance-cos' || $uri == 'trainer-database' || $uri == 'auditor-database' || $uri == 'course-database' || $uri == 'training-material' || $uri == 'vendor-database' || $uri == 'government-projects' || $uri == 'certified-training' || $uri == 'content' || $uri =='assessment-center-pre-or-post')
			{
				$mats_id = Materials::get_materials_id($uri);
			}
			else
			{
				$cats_id = Industries::get_industry_id($uri_segment[6]);
			}
			//$cats_id = Industries::get_industry_id($uri_segment[6]);
		}
		else
		...
		// {
		// 	$mats_id = Materials::get_materials_id($uri_segment[7]);
		// }
		// else
		// {
		// 	$mats_id = '';
		...
		// {
		// 	$tools_id = Tools::get_tools_id($uri_segment[8]);
		// }
		// else
		// {
		// 	$tools_id = '';
		...
		// if(!empty($uri_segment[9]))
		// {
		// 	$microf_id = Microfinance::get_microfinance_id($uri_segment[9]);
		// }
		// else
		// {
		// 	$microf_id = '';
		...
		{
			$data = $cats_id;
			array_push($data, "cat_id");
		}
		if (!empty($mats_id)) 
		{
			$data = $mats_id;
			array_push($data, "partnr_id");
		}
		// if(!empty($tools_id))
		// {
		// 	$data = $tools_id;
		// 	array_push($data, "tool_id");
		// }
		// if (!empty($microf_id)) 
		// {
		// 	$data = $microf_id;
		// 	array_push($data, "microf_id");
		// }
		// print_r($data);
		// echo print_r($data); exit;
		$total_count = Question::count_feed_for($current_user->id,$query," ");
		$pagination = new Pagination($page, $per_page, $total_count);
		$questions = Question::get_feed_for($current_user->id ,$query," LIMIT {$per_page} OFFSET {$pagination->offset()} ", $data );
